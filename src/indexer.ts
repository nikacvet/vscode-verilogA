// is used to provide features like "Go to Definition," "Find References," and "Symbol Highlighting" 
...
export class VerilogAIndexer {
     ...
     */
    public symbols: Map<string, Array<VerilogASymbol>>;
    public mostRecentSymbols: Array<VerilogASymbol>;
    public building = false;
    public statusbar: StatusBarItem;
    public parser: VerilogAParser;
    ...
    constructor(statusbar: StatusBarItem, parser: VerilogAParser, channel: OutputChannel) {
        this.statusbar = statusbar;
        this.parser = parser;
        this.outputChannel = channel;
        this.symbols = new Map<string, Array<VerilogASymbol>>();
    }

    public initialize() {
        const settings = workspace.getConfiguration();
        this.parallelProcessing = settings.get('veriloga.parallelProcessing');
        this.forceFastIndexing = settings.get('veriloga.forceFastIndexing');
        this.maxLineCountIndexing = settings.get('veriloga.maxLineCountIndexing');
        this.documentSymbolPrecision = settings.get('veriloga.documentSymbolsPrecision');
    }
    ...
    */
    public async build_index(): Promise<any> {
        let cancelled = false;
        this.symbolsCount = 0;
        this.statusbar.text = 'Verilog A: Indexing..';
        this.initialize();
        ...
        return window
            ...
            .then(() => {
                ...
                this.building = false;
                if (cancelled) {
                    this.statusbar.text = `Verilog A: Indexing cancelled at ${this.symbolsCount} indexed objects`;
                } else {
                    this.statusbar.text = `Verilog A: ${this.symbolsCount} indexed objects`;
                }
            });
    }
    ...
    public async find_files(token: CancellationToken): Promise<Uri[]> {
        ...
        return new Promise(async (resolve) => {
            const settings = workspace.getConfiguration();
            const globArray: string[] = settings.get('veriloga.includeIndexing');
            const exclude: string = settings.get('veriloga.excludeIndexing');
            let uris: Uri[] = [];
    ...
    */
    public async processFile(uri: Uri, total_files = 0) {
        ...
        return new Promise(async (resolve) => {
            ...
            .then((output: Array<VerilogASymbol>) => {
                if (output.length > 0) {
            ...
            .catch((error) => {
                this.outputChannel.appendLine(`Verilog A: Indexing: Unable to process file: ${uri.toString()}`);
    ...
    */
    public async onChange(document: TextDocument): Promise<any> {
        return new Promise(() => {
            if (!isVerilogADocument(document)) {
                return;
            }
            if (!workspace.getConfiguration().get('veriloga.enableIncrementalIndexing')) {
                return;
            }
            if (
                !minimatch(
                    document.uri.toString(),
                    workspace.getConfiguration().get('veriloga.excludeIndexing').toString()
                )
            ) {
    ...
    */
    addDocumentSymbols(document: TextDocument, symbolsMap: Map<string, Array<VerilogASymbol>>): Thenable<number> {
        // eslint-disable-next-line no-async-promise-executor
        return new Promise(async (resolve) => {
            if (!document || !symbolsMap) {
                resolve(new Array<VerilogASymbol>());
                return;
            }
    ...
    */
    removeDocumentSymbols(fsPath: string, symbolsMap: Map<string, Array<VerilogASymbol>>): number {
    ...
    */
    updateMostRecentSymbols(recentSymbols: Array<VerilogASymbol>): void {
        if (this.mostRecentSymbols) {
            ...
            let maxSymbols = new Array<VerilogASymbol>();
