...
import { isVerilogADocument, getLineRange } from '../utils/server';
    ...
    */
    public getTextDocumentDiagnostics(document: TextDocument): Thenable<Map<string, Diagnostic[]>> {
        return new Promise((resolve, reject) => {
            if (!document) {
                reject(new Error('Verilog A: Invalid document.'));
                return;
            }

            if (!isVerilogADocument(document)) {
                reject(new Error('The document is not a Verilog A file.'));
                return;
            }
            ...
            if (
                this.configurations.get(this.compilerConfigurationsKeys[0]) === 'OpenVAF' &&
            ...
            } else if (this.configurations.get(this.compilerConfigurationsKeys[0]) === 'OpenVAF') {
