...
export class VerilogAWorkspaceSymbolProvider implements WorkspaceSymbolProvider {
    /*
     ...
     */
    public indexer: VerilogAIndexer;
    ...
    */
    public provideWorkspaceSymbols(query: string, token: CancellationToken): Thenable<Array<VerilogASymbol>> {
        return new Promise((resolve, _reject) => {
            if (query === undefined || query.length === 0) {
                ...
                const results = new Array<VerilogASymbol>();
    ...
    public getAllModules(): Thenable<Array<VerilogASymbol>> {
        return new Promise((resolve) => {
            let modules: Array<VerilogASymbol> = [];
            this.indexer.symbols.forEach((list) => {
    ...
    private uniquifyResults(results: Array<VerilogASymbol>) {
    ...
    */
    public provideWorkspaceModule(query: string): VerilogASymbol {
        if (query.length === 0) {
            return undefined;
        }
        let symbolInfo;
