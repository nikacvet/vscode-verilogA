// provides custom parsing abilities that extend the antlr generated parser
...
export class VerilogAParser {
    private illegalMatches =
        /(?!\breturn\b|\bbegin\b|\bend\b|\belse\b|\bjoin\b|\bfork\b|\bfor\b|\bif\b|\bautomatic\b|\bgenerate\b|\bassign\b|\binitial\b|\bassert\b|\bdisable\b)/;
    ...
    */
    public get_all_recursive(
        ...
        parent?: string,
        depth = 0
    ): Array<VerilogASymbol> {
        let symbols: Array<VerilogASymbol> = [];
        ...
        for (let i = 0; i < regexes.length; i++) {
            ...
            while (true) {
                ...
                if (!isCommented) {
                    const symbolInfo = new VerilogASymbol(match.groups.name, type, parent, location);
    ...
    private get_ports(document: TextDocument, text: string, offset, parent): Thenable<Array<VerilogASymbol>> {
        return new Promise((resolve) => {
            const symbols: Array<VerilogASymbol> = [];
            // eslint-disable-next-line no-constant-condition
            while (true) {
                ...
                const symbolInfo = new VerilogASymbol(
    ...
    private get_params(document: TextDocument, text: string, offset, parent): Thenable<Array<VerilogASymbol>> {
        return new Promise((resolve) => {
